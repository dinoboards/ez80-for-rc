...
#define HLinstr(opcode, cycles, morecycles)                                                                                        \
  case opcode: {                                                                                                                   \
    ...
    if (ixoriy == 0)                                                                                                               \
      ...
      tstates += morecycles, addr = (ixoriy == 1 ? ix : iy) + (signed char)fetch(pc), pc++
#define endinstr                                                                                                                   \
  }                                                                                                                                \
  ;                                                                                                                                \
  break
...
#define setxh(x) (ixoriy == 0 ? (h = (x)) : ixoriy == 1 ? (ix = (ix & 0xff) | ((x) << 8)) : (iy = (iy & 0xff) | ((x) << 8)))
#define setxl(x) (ixoriy == 0 ? (l = (x)) : ixoriy == 1 ? (ix = (ix & 0xff00) | (x)) : (iy = (iy & 0xff00) | (x)))

#define inc(var) /* 8-bit increment */                                                                                             \
  (var++, f = (f & 1) | (var & 0xa8) | ((!(var & 15)) << 4) | ((!var) << 6) | ((var == 128) << 2))
#define dec(var) /* 8-bit decrement */                                                                                             \
  (f = (f & 1) | ((!(var & 15)) << 4) | 2, --var, f |= (var & 0xa8) | ((var == 127) << 2) | ((!var) << 6))
#define swap(x, y)                                                                                                                 \
  {                                                                                                                                \
    unsigned char t = x;                                                                                                           \
    x               = y;                                                                                                           \
...
#define suba(x, c) /* 8-bit subtract */                                                                                            \
  do {                                                                                                                             \
    unsigned short y;                                                                                                              \
    unsigned char  z = (x);                                                                                                        \
    y                = (a - z - (c)) & 0x1ff;                                                                                      \
    f                = (y & 0xa8) | (y >> 8) | (((a & 0x0f) < (z & 0x0f) + (c)) << 4) | (((a ^ z) & 0x80 & (y ^ a)) >> 5) | 2;     \
    f |= (!(a = y)) << 6;                                                                                                          \
  } while (0)
#define cpa(x) /* 8-bit compare */                                                                                                 \
  do {                                                                                                                             \
    unsigned short y;                                                                                                              \
    unsigned char  z = (x);                                                                                                        \
    y                = (a - z) & 0x1ff;                                                                                            \
...
#define anda(x) /* logical and */                                                                                                  \
  do {                                                                                                                             \
    a &= (x);                                                                                                                      \
    f = (a & 0xa8) | ((!a) << 6) | 0x10 | parity(a);                                                                               \
  } while (0)
#define xora(x) /* logical xor */                                                                                                  \
  do {                                                                                                                             \
    a ^= (x);                                                                                                                      \
    f = (a & 0xa8) | ((!a) << 6) | parity(a);                                                                                      \
  } while (0)
#define ora(x) /* logical or */                                                                                                    \
  do {                                                                                                                             \
    a |= (x);                                                                                                                      \
    f = (a & 0xa8) | ((!a) << 6) | parity(a);                                                                                      \
  } while (0)

#define jr /* execute relative jump */                                                                                             \
  do {                                                                                                                             \
...
#define call /* execute call */                                                                                                    \
  do {                                                                                                                             \
    tstates += 7;                                                                                                                  \
    push2(pc + 2);                                                                                                                 \
    jp;                                                                                                                            \
  } while (0)
#define ret /* execute return */                                                                                                   \
  do {                                                                                                                             \
    tstates += 6;                                                                                                                  \
    pop2(pc);                                                                                                                      \
  } while (0)
#define pop2(var)    /* pop 16-bit register */ (var = fetch2(sp), sp += 2)
#define pop1(v1, v2) /* pop register pair */ (v2 = fetch(sp), v1 = fetch(sp + 1), sp += 2)
#define push2(val)   /* push 16-bit register */                                                                                    \
  do {                                                                                                                             \
    sp -= 2;                                                                                                                       \
    store2(sp, (val));                                                                                                             \
  } while (0)
#define push1(v1, v2) /* push register pair */                                                                                     \
  do {                                                                                                                             \
    sp -= 2;                                                                                                                       \
    store2b(sp, v1, v2);                                                                                                           \
  } while (0)
...
instr(39, 4);
{
  ...
  if (f & 2) /* if N */
  {
    ...
    if ((f & 16) || (temp > 9)) { /* adjust low digit */
      if (temp > 5)
        f &= ~16;
      acu -= 6;
      acu &= 0xff;
