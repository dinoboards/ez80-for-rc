...
char *GetErrorStr(dword dwErr) {
  ...
  FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS |
                ...
                NULL, dwErr, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&lpMsgBuf, sizeof(ErrStr), NULL);
...
int redir_ofile(cpm_byte *fcb, char *s) {
  ...
  if (!redir_ro_fcb(fcb)) {
    ...
    h = (int)CreateFile(s, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
...
void redir_log_fcb(cpm_byte *fcb) {
  ...
  if (drv && drv != '?')
    ...
    redir_log_drv(redir_cpmdrive);
}

int redir_ro_drv(cpm_byte drv) {
  if (!drv)
    return redir_ro_drives & 1;
  else
    return redir_ro_drives & (1L << drv);
}
...
int redir_ro_fcb(cpm_byte *fcb) {
  ...
  if (drv && drv != '?')
    return redir_ro_drv(drv - 1);
  else
    return redir_ro_drv(redir_cpmdrive);
}

cpm_word redir_xlt_err(void) {
  if (redir_password_error())
    return 0x7FF; /* DRDOS pwd error */
  ...
  case EBADF:
    return 9; /* Bad FCB */
  case EINVAL:
    return 0x03FF; /* Readonly file */
  case EPIPE:
    return 0x01FF; /* Broken pipe */
  case ENOSPC:
    return 1; /* No space */
  default:
    return 0xFF; /* Software error */
  }
}
