...
static char *drive_to_hostdrive(int cpm_drive) {
  ...
  if (!redir_drive_prefix[cpm_drive])
    return NULL;
  dw = GetFullPathName(redir_drive_prefix[cpm_drive], sizeof(prefix), prefix, &lpfp);
...
cpm_byte fcb_getdrv(void) { return redir_cpmdrive; }

cpm_byte fcb_user(cpm_byte usr) {
  if (usr != 0xFF)
    redir_cpmuser = usr % 16;
...
cpm_word fcb_logvec(void) { return redir_l_drives; }

cpm_word fcb_rovec(void) { return redir_ro_drives; }

cpm_word fcb_rodisk(void) {
  cpm_word mask = 1;

  if (redir_cpmdrive)
    mask = mask << redir_cpmdrive;
...
#else
  sync();
  return 0; /* Apparently some sync()s are void not int */
#endif
}
