...
cpm_word fcb_open(cpm_byte *fcb, cpm_byte *dma) {
  ...
  {
    if (fcb[0x0C])
      FCBRET(0x0BFF); /* Can't assign "floating" dir */
  ...
  if (fcb[0x20] == 0xFF)
    fcb[0x20] = fcb[LENGTH_OFFSET] & 0x7F;
...
cpm_word fcb_write(cpm_byte *fcb, cpm_byte *dma) {
  ...
  if (len < npos)
    redir_wr32(fcb + LENGTH_OFFSET, npos);

  if (rv < 0)
    FCBRET(redir_xlt_err()); /* error */
  if (rv < redir_rec_len)
    FCBRET(1); /* disk full */
  FCBRET(0);
}
...
cpm_word fcb_creat(cpm_byte *fcb, cpm_byte *dma) {
  ...
  handle = (int)CreateFile(fname, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS,
...
#else
  DBGMSGV("create file '%s'\n", fname);
  handle = open(fname, O_RDWR | O_CREAT | O_EXCL | O_BINARY, S_IREAD | S_IWRITE);
  if (handle < 0) {
...
cpm_word fcb_rename(cpm_byte *fcb, cpm_byte *dma) {
  ...
  /* Don't support ambiguous filenames */
  if (redir_fcb2unix(fcb, ofname))
    FCBRET(0x09FF);
  if (redir_fcb2unix(fcb + 0x10, nfname))
    FCBRET(0x09FF);
  ...
  if (fcb[0] & 0x80)
    FCBRET(0xFF); /* Can't rename directories */
  ...
  sdrv = fcb[0] & 0x7F;
  if (!sdrv)
    sdrv = redir_cpmdrive + 1;
  ddrv = fcb[0x10] & 0x7F;
  if (!ddrv)
    ddrv = redir_cpmdrive + 1;
...
cpm_word fcb_randrd(cpm_byte *fcb, cpm_byte *dma) {
  ...
  if (zxlseek(handle, offs, SEEK_SET) < 0)
    FCBRET(6); /* bad record no. */
  ...
  for (n = rv; n < rd_len; n++)
    dma[n] = 0x1A; /* pad last read to 128 boundary with 0x1A*/
...
cpm_word fcb_chmod(cpm_byte *fcb, cpm_byte *dma) {
  ...
  if (fcb[9] & 0x80)
    omode |= FILE_ATTRIBUTE_READONLY;
  if (fcb[10] & 0x80)
    omode |= FILE_ATTRIBUTE_SYSTEM;
  if (!(fcb[11] & 0x80))
    omode |= FILE_ATTRIBUTE_ARCHIVE;

  if (!omode)
    omode = FILE_ATTRIBUTE_NORMAL;
...
#ifdef __MSDOS__
  if (redir_drdos)
    return 0xA1; /* Supports passwords & Update stamps */
