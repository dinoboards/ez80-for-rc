...
static struct dirent *next_entry(DIR *dir, cpm_byte *fcb, cpm_byte *pattern, struct stat *st) {
  struct dirent *en;
  int            unsatisfied;
  int            drv = fcb[0] & 0x7F;
...
void volume_label(int drv, cpm_byte *dma) {
  ...
  if (redir_drdos)
    dma[0x0c] |= 0x80; /* Under DRDOS, passwords allowed */
#else
...
cpm_word fcb_unlink(cpm_byte *fcb, cpm_byte *dma) {
  ...
  if (fcb[5] & 0x80)
    unpasswd = 1; /* Remove password rather than file */
...
cpm_word redir_get_label(cpm_byte drv, char *pattern) {
  ...
  while (!done) {
    if ((fblk.ff_attrib & FA_LABEL) && !(fblk.ff_attrib & (FA_SYSTEM | FA_HIDDEN))) {
