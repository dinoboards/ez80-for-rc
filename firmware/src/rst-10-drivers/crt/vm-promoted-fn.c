...
2. Add an else condition, to include required files  "../../../q3vm/includes/host-functions.h", "../vm-promoted-fn.h" and any others required
...
7. In link_vm_globals, assign global pointers to the correct mapped address (see generated vm/bytecode.map file for vm virtaul address to be applied)
...
void link_vm_globals(vm_t *vm) {
  _counter = &vm->dataBase[0x1e4];
